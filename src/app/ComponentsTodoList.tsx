...
export function setdesign(field: number) {
  switch (field) {                        // 일정분야에 따른 일정리스트 디자인 변경 (분야문자열, 문자색깔, 아이콘)
    case 3:
      return ({
        icon: "lock-alt.svg",
...
function Todo({ calData, clear }: { calData: calObject, clear: boolean}) {
  ...
  const openWriteTodo = () => {
    window.open(`/writeTodoPopup?year=${calData.day.slice(0, 4)}&month=${calData.day.slice(5, 7)}&day=${calData.day.slice(8, 10)}$calData=${JSON.stringify(calData)}`,'_blank','width=400, height=300')
  }
    ...
    <div className=''>
      {!isModify && <div className={`${todoBox} text-skyblue text-orange text-green`}>
        ...
        <div className='flex-auto flex justify-end space-x-[10px]'>
          <button className="w-[30px]" onClick={openWriteTodo} >
            <img src="ICON_pencile.svg" alt="image description" />
          </button>
          <div className="flex items-center mb-[4px]">
            <input id="default-checkbox" type="checkbox" value="" className="w-[20px] h-[20px] bg-pink border-deepBlue ring-blue rounded focus:ring-blue-500 dark:ring-offset-gray-800 focus:ring-2 dark:bg-blue-100 dark:border-gray-600 disabled:bg-red" />
          </div>
        </div>
      </div>}
      {isModify && modifyTodo}
...
function TodoList(calData: calObject[], setDay: number) {
  const [isModify, setIsModify] = useState(false);
  let modifying = () => {
    setIsModify(!isModify);
  }
  ...
  let todo = /*setDay ? calData.filter((e: { check: boolean; day: string }) => (!e.check && (Number(e.day.slice(8, 10)) == setDay))).map((key: calObject) => (<div id={`${key.id}`}><Todo calData={key} /></div>)) : */calData.filter((e: { check: boolean; }) => (!e.check)).map((key: calObject) => (<div id={`todoComponent${key.id}`}><Todo calData={key} clear={false}/></div>));
  let clearTodo = /*setDay ? calData.filter((e: { check: boolean; day:string}) => (e.check && (Number(e.day.slice(8, 10)) == setDay))).map((key: calObject) => (<div id={`${key.id}`}><ClearTodo calData={key} /></div>)) : */calData.filter((e: { check: boolean; }) => (e.check)).map((key: calObject) => (<div id={`todoComponent${key.id}`}><Todo calData={key} clear={true}/></div>));
  //console.log(todo);
  return (
    ...
    <div className={`${todoListContainer} koreanFont`}>
      <div className="flex justify-normal">
        ...
        {adminPower && <div className="flex-auto flex justify-end space-x-[10px]">
          ...
          <button className="" onClick={changeShowNewTodoHandler}>
            <img src="/BTN_write.svg" alt="설명" />
          </button>
        </div>}
      </div>
      {showNewTodo && createTodo}
      {TodoList(calendarData, day)}
