    ...
    return (
        <div>
            ...
            <div className="bg-deepBlue min-w-[500px] w-full h-auto p-[20px] rounded-lg">
                <div className="flex items-ceter text-orange text-[20px] font-bold ">
                <div className="w-[200px] text-ceter">책 이름</div>
                </div>
                {bookList.length ? bookList.map((key: bookObject) => (<div className="flex items-ceter"><div className="w-[200px]">{key.name}</div>{IconBook('w-[20px] h-[20px]', `${key.rental ? `green` : `blue`}`)}</div>)) : '책이 존재하지 않습니다.'}
                {addBook &&
                    <div>
